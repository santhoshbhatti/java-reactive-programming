...
public class Sec06PublishOnSubscribeOnDemo {
	...
	public static void main(String[] args) {
		...
		//executed on that subscribeOn's schedulers thread pool.
		//The subscribeOn nearest to the operator or the publishers is 
		//taken into account got scheduling that operator or the
		//publisher
		
		//publishOn and subscribeOn are executed once in the 
		//pipeline and their effect on execution remains till
		//the execution ends ......they are not changed again
		//once the oipeline is setup and executing these do not changes
		//Once invoked these do not change
		flux.doFirst(() -> printThreadName("first2"))
				.publishOn(Schedulers.newParallel("pubon",2,true))
				.doOnNext(i -> printThreadName("nextElement2: "+i))
				.subscribeOn(Schedulers.newParallel("subon",2,true))
